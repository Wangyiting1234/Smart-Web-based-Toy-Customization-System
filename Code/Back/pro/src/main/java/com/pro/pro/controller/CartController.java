...
public class CartController extends BaseController{
    ...
    public JsonResult<Void> addCart(Integer productId,Integer productNum,String size,String color, HttpSession session){
            Integer userId = getuidFromSession(session);
            String userTel = getUsertelFromSession(session);
            cartService.addNewCart(userId,productId,productNum,size,color,userTel);
            return new JsonResult<>(success);
    }
    @RequestMapping("cartlist")
    public JsonResult<List<Cart>> getByUserId(HttpSession session){
        Integer userId= getuidFromSession(session);
        List<Cart> data = cartService.getByUserId(userId);
